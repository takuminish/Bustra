...
public class Board extends Panel{
		...
		this.height = height;                             // 指定した値を縦のドロップの数にするよ
    	 ...
    	 for (int k1 = 0; k1 < this.height; k1++) {
    		 ...
    		 for (int k2 = 0; k2 < this.width; k2++) {
    			 ...
    			 case 3: this.board.get(k1).add(new DarkDrop(k2 * 80, k1 * 80)); break;  // ランダムの値が3だったら闇ドロップを格納するよ
    			 case 4: this.board.get(k1).add(new LightDrop(k2 * 80, k1 * 80)); break; // ランダムの値が4だったら光ドロップを格納するよ
    			 }
    		 }
    	 }
    }
	...
	public void paintComponent(Graphics g) {
		this.setBounds(0, 0, this.width * 80, this.height * 80);  // 指定した位置と大きさで盤面を表示するよ
		// 二重for文でDropを表示していくよ
		for(int k1 = 0; k1 < this.height; k1++) {
			for (int k2 = 0; k2 < this.width; k2++) {
